<?php
    ...
    if(isset($_SESSION['langue']) && $_SESSION['langue'] == 'EN') {
        ...
        if(!isset($_POST['Sauvegarder'])) {
            header('Location: ./modification.php?msgErreur=Please fill in the fields');
            exit;
        }
    
        if (empty($_POST['langues']) || empty($_POST['theme']) || empty($_POST['monnaie'])) {
            header('Location: ./modification.php?msgErreur=Incorrect selection');
            exit;
        }
        ...
        if($mdpActeur != $confmdpActeur){
            header('Location: ./modification.php?msgErreur=Different passwords');
            exit;
        }
    
        if (!preg_match('#^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$#', $mailActeur)) {
            header('Location: ./modification.php?msgErreur=Check your email address');
            exit;
        }
        ...
        echo('<script language="JavaScript" type="text/javascript"> 
                alert("Modification successful. Please log in again."); 
