<?php
    ...
    if(isset($_SESSION['langue']) && $_SESSION['langue'] == 'EN') {
        ...
        if(!isset($_POST['Sauvegarder'])) {
            header('Location: ./updateProduit.php?msgErreur=Please fill in the fields');
            exit;
        }
    
        if (empty($_POST['categorie']) || empty($_POST['nomArticle'])) {
            header('Location: ./updateProduit.php?msgErreur=Incorrect selection');
            exit;
        }
        ...
        if(!preg_match('#^[0-9]{1,8}[,.][0-9]{2}$#', $prixProduit)) {
            header('Location: ./updateProduit.php?msgErreur=Check your price entry');
            exit;
        }
    
        if(!preg_match('#^[0-9]{1,2}$#', $prixPromoProduit)) {
            header('Location: ./updateProduit.php?msgErreur=Check the entry of your promotional price');
            exit;
        }
